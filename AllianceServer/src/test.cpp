...
public:
    bool init() {
        // 会和当前进程建立连接的所有服务器
        setConnectServerType("AllianceServerC");
    ...
    virtual bool checkConnect() {
        ...
        if (!checkConnectedServer("AllianceServerC"))
            if (!connectServerByType("AllianceServerC"))
                ret = false;
    ...
    virtual void v_closeServer(xNetProcessor *np) {
        ...
        if (strcmp(np->m_arrcName, "AllianceServerC") == 0) {
            // 处理Server断开之后的逻辑
        }
...
int main(int argc, char *argv[]) {
    ...
    while ((oc = getopt(argc, argv, "dn:")) != -1) {
        ...
        case 'n':
        {
            myServer = new MyServer("AllianceServer", optarg);
        }
        break;
        case '?':
            break;
