...
public:
    bool init() {
        // 会和当前进程建立连接的所有服务器
        setConnectServerType("FriendServerC");
    ...
    virtual bool checkConnect() {
        ...
        if (!checkConnectedServer("FriendServerC"))
    ...
    virtual void v_closeServer(xNetProcessor *np) {
        ...
        if (strcmp(np->m_arrcName, "FriendServerC") == 0) {
...
int main(int argc, char *argv[]) {
    ...
    while ((oc = getopt(argc, argv, "dn:")) != -1) {
        ...
        case 'n':
        {
            myServer = new MyServer("FriendServer", optarg);
        }
        break;
        case '?':
            break;
