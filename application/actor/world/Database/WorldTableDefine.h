...
#pragma pack(1)
struct WorldObjectQueryData : public DatabaseTableCol
{
    uint64 orderid = 0;
    uint64 steamid = 0;
    uint32 cinfigid = 0;
    ...
    WorldObjectQueryData() {
        CleanUp();
    }
    void CleanUp() {
        bzero(this, sizeof(WorldObjectQueryData));
    }
...
// 数据库结构定义
#define WorldDBColDefine() \
static dbCol WorldUpdateStateCol[] = {\
